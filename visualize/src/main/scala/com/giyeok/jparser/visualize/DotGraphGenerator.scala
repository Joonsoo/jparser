...
//    }
//
//    private var _nodes = Seq[(State, DotGraphNode)]()
//    private val edges = scala.collection.mutable.Map[Expectation, DotGraphEdge]()
//
//    private var nodeNamesMap = Map[State, String]()
//    def nodeNameOf(node: State): String = {
...
//    }
//
//    def getNode(node: State): Option[DotGraphNode] = (_nodes find { _._1 == node } map { _._2 })
//    def addNode(node: State): DotGraphNode = {
//        getNode(node) match {
...
//    }
//
//    def addGraph(graph: ParsingContext): DotGraphGenerator = {
//        var visited = Set[State]()
//        var queue = List[State]()
//        def traverseNode(): Unit = {
//            val node = queue.head
//            queue = queue.tail
//            addNode(node)
//            graph.expectsByStart(node).toSeq sortWith { (x, y) => x.end.kernel.symbolId < y.end.kernel.symbolId } foreach { edge =>
...
//            }
//        }
//        val startNode = State(Kernel(ngrammar.startSymbol, 0, 0, 0)(ngrammar.nsymbols(ngrammar.startSymbol)), Always)
//        visited += startNode
//        queue +:= startNode
//        traverseNode()
//
...
//    }
//
//    def addTransition(baseGraph: ParsingContext, afterGraph: ParsingContext): DotGraphGenerator = {
//        addGraph(baseGraph)
//        // baseGraph -> afterGraph 과정에서 없어진 노드/엣지 스타일에 dotted 추가
//        (baseGraph.states -- afterGraph.states) foreach { removedNode =>
//            getNode(removedNode) match {
//                case Some(node) => node.addStyle("dotted")
//                case _ => // nothing to do
//            }
//        }
//        (baseGraph.expects -- afterGraph.expects) foreach { removedEdge =>
//            if (edges contains removedEdge) {
//                edges(removedEdge).addStyle("dotted")
//            }
//        }
//        // 새로 추가된 노드 스타일에 filled 추가
//        (afterGraph.states -- baseGraph.states) foreach { node =>
//            addNode(node).addStyle("filled")
//        }
//        (afterGraph.expects -- baseGraph.expects) foreach { edge =>
...
//        edges foreach { kv =>
//            kv._1 match {
//                case edge @ Expectation(start, end, actual) =>
//                    // TODO actual 표현
