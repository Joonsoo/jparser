...
//
//    val graphsMap = scala.collection.mutable.Map[(Int, Int, Boolean), (Preprocessed, Map[TermGroupDesc, (Preprocessed, Set[SequenceKernel])])]()
//
...
//            case SymbolKernel(symbolId, beginGen) if beginGen < 0 =>
//                fig.horizontalFig(Spacing.Big, Seq(
//                    fig.textFig(s"$symbolId", appear.small),
//                    symbolFigure(grammar, symbolId)))
//            case SequenceKernel(sequenceId, pointer, beginGen, endGen) if beginGen < 0 && endGen < 0 =>
//                fig.horizontalFig(Spacing.Big, Seq(
...
//}
//
//class PreprocessedSlicedDerivationGraphWidget(parent: Composite, style: Int, fig: NodeFigureGenerators[Figure], grammar: NGrammar, preprocessed: Preprocessed, sliced: (Preprocessed, Set[SequenceKernel]))
