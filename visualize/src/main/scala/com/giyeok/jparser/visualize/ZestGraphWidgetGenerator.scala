...
//
//    val nodesMap: mutable.Map[State, CGraphNode] = scala.collection.mutable.Map[State, CGraphNode]()
//    val edgesMap: mutable.Map[Expectation, Seq[GraphConnection]] = scala.collection.mutable.Map[Expectation, Seq[GraphConnection]]()
//    var visibleNodes: Set[State] = Set[State]()
//    var visibleEdges: Set[Expectation] = Set[Expectation]()
//
//    def addNode(grammar: NGrammar, node: State): Unit = {
...
//    def addEdge(edge: Expectation): Unit = {
//        if (!(edgesMap contains edge)) {
//            val Expectation(start, end, actual) = edge
...
//    }
//
//    def setVisibleSubgraph(nodes: Set[State], edges: Set[Expectation]): Unit = {
//        visibleNodes = nodes
//        visibleEdges = edges
//
//        nodesMap foreach { kv =>
...
//    val blinkCycle = 10
//    val highlightedNodes = scala.collection.mutable.Map[State, (Long, Int)]()
//
//    var _uniqueId = 0L
//    def uniqueId: Long = { _uniqueId += 1; _uniqueId }
//
//    def highlightNode(node: State): Unit = {
...
//                                unhighlightAllNodes()
//                                nodesSeq.asInstanceOf[Seq[State]] foreach { highlightNode }
...
//    initialize()
//
//    def getTooltips: Map[State, Seq[Figure]] = {
//        var tooltips = Map[State, Seq[Figure]]()
...
//        tooltips
//    }
//    protected def initializeTooltips(tooltips: Map[State, Seq[Figure]]): Unit = {
//        tooltips foreach { kv =>
//            val (node, figs) = kv
...
//                case 'S' | 's' =>
//                    // Stat
//                    println(s"Nodes: ${graph.states.size} Edges: ${graph.expects.size}")
...
//        // 변경이 있는 부분은 굵은 선
//        // 없어지는 노드, 엣지는 주황색 점선
//        (baseGraph.states -- nextGraph.states) foreach { removedNode =>
...
//        }
//        (baseGraph.expects -- nextGraph.expects) foreach { removedEdge =>
...
//    def setTipNodeBorder(node: State): Unit = {
//        val shownNode = nodesMap(node)
