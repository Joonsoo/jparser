...
## 현재 benchmark (size=1000, commit d73157dd 기준)

| case             | m2 (Scala) | kt (port) | m3       | m3/kt |
| ---------------- | ---------- | --------- | -------- | ----- |
| cfg_only         | 3.07ms     | 2.57ms    | 1.86ms   | 0.72x |
| longest_only     | 9.15ms     | 5.57ms    | 8.73ms   | 1.57x |
| lookahead_only   | 5.03ms     | 4.52ms    | 4.37ms   | 0.97x |
| longest_join     | OOM        | OOM       | 10.48ms  | —     |
| cond_heavy       | OOM        | 5.54ms    | 8.15ms   | 1.47x |

cfg_only / lookahead_only 는 m3 가 더 빠름. longest_only / cond_heavy 가 1.47-1.57x — 남은 cost 가 어디 있는지 분석 필요.

(cfg_only / longest_only 는 prior 측정값. 최신 measurement 의 ConsoleLauncher stdout 이 앞 3 case 를 buffer / 누락. JUnit reporter 직접 사용 또는 file output 으로 측정 권장.)
