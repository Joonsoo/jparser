...
5. **AcceptCondition.referencedRoots + hasFromNextGen metadata** (7ca3ebed)
   ...
   - `hasFromNextGen` 는 미사용 — (5) dirty-root skip 의 prerequisite.
...
## 현재 benchmark (size=1000, 마지막 측정)

| case             | m2 (Scala) | kt (port) | m3       | m3/kt |
| ---------------- | ---------- | --------- | -------- | ----- |
| cfg_only         | 3.07ms     | 2.57ms    | 1.86ms   | 0.72x |
| longest_only     | 9.15ms     | 5.57ms    | 8.73ms   | 1.57x |
| lookahead_only   | 4.99ms     | 4.95ms    | 5.34ms   | 1.08x |
| longest_join     | OOM        | OOM       | 10.82ms  | —     |
| cond_heavy       | OOM        | 5.38ms    | 8.57ms   | 1.59x |

cfg_only / lookahead_only 는 m3 가 거의 또는 더 빠름. longest_only / cond_heavy 가 1.5-1.6x — 남은 cost 가 어디 있는지 분석 필요.

## 남은 개선 아이디어

### High priority (시도할만)

#### (5) Dirty-root evolve skip
- `referencedRoots` + `hasFromNextGen` 활용.
- `affectedRoots = condPathFinishes.keys + 이번 step active/inactive 변경된 cond roots`.
- 조건: `!cond.hasFromNextGen && cond.referencedRoots.isDisjoint(affectedRoots)`.
- 만족하면 evolve skip — condition 그대로 reuse.
- cond_heavy 에서 step5 evolve 가 14% 차지. 일부 skip 되면 효과 큼.
- 위험: correctness 검증 신중. condition unchanged 가 정말 안전한지 확인.
...
- `ParserBenchmarkTaxonomyTest` (Disabled) — 5 카테고리 m2/kt/m3 비교.
- `Mgroup3ProfilingTest` (Disabled) — sampling profiler + phase timing.
- 두 test 모두 `@Disabled` 해제 후 `runParserBenchmarkTaxonomy` / `runMgroup3Profile` action 으로 실행.
