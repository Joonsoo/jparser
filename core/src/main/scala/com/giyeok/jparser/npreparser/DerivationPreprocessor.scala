...
//import com.giyeok.jparser.nparser.ParsingContext.State
...
//    }
//
//    case class Preprocessed(base: State, lifted: Cont, baseTasks: List[ProgressTask], nextGraph: Graph, nextDeriveTips: Set[State]) {
...
//            // tasks
//            val base1 = State(Kernel(base.kernel.symbolId, base.kernel.pointer, beginGen, endGen)(base.kernel.symbol), condition)
//            val nodeMapper = { (node: State) =>
//                node match {
//                    case `base` => base1
...
//            val updated1 = lifted.updatedFinalStatesMap map { kv =>
...
//
//    @tailrec private def rootBlockingRec(nextGen: Int, root: State, tasks: List[Task], cc: Cont, rootTasks: List[ProgressTask]): (Cont, List[ProgressTask]) =
//        tasks match {
...
//        }
//
//    @tailrec private def deriveBlockingRec(nextGen: Int, root: State, tasks: List[Task], cc: Cont, rootTasks: List[ProgressTask], tipsTasks: List[DeriveTask]): (Cont, List[ProgressTask], List[DeriveTask]) =
//        tasks match {
//            case (task @ ProgressTask(`root`, _)) +: rest =>
//                deriveBlockingRec(nextGen, root, rest, cc, task +: rootTasks, tipsTasks)
//            case (FinishTask(`root`)) +: _ =>
//                ??? // should not happen
//            case (task @ DeriveTask(State(kernel, _))) +: rest if kernel.pointer > 0 =>
...
//                val baseNode = State(Kernel(symbolId, pointer, 0, 0)(grammar.symbolOf(symbolId)), Always)
...
//    }
//
//    def sliceOf(node: State, input: Input): Option[Preprocessed] = {
//        val State(kernel, condition) = node
