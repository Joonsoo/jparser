...
//    case class Preprocessed(baseNode: Node, graph: Graph, baseFinishes: Seq[(AcceptCondition, Option[Int])], baseProgresses: Seq[AcceptCondition]) {
...
//            Preprocessed(baseNode.shiftGen(gen), graph.shiftGen(gen), baseFinishes, baseProgresses)
//        }
//    }
//}
//
...
//        node match {
//            case SymbolKernel(symbolId, _) => symbolDerivationOf(symbolId)
//            case SequenceKernel(sequenceId, pointer, _, _) => sequenceDerivationOf(sequenceId, pointer)
//        }
//    }
//
...
//
//    def termNodesOf(node: Node): Set[SymbolKernel] = {
//        node match {
//            case node @ SymbolKernel(symbolId, _) => symbolTermNodesOf(symbolId)
//            case node @ SequenceKernel(sequenceId, pointer, _, _) => sequenceTermNodesOf(sequenceId, pointer)
//        }
//    }
//}
//
...
//
//    def sliceOf(node: Node, input: Input): Option[(Preprocessed, Set[SequenceKernel])] = {
//        val slicedMap = node match {
//            case SymbolKernel(symbolId, _) => symbolSliceOf(symbolId)
//            case SequenceKernel(sequenceId, pointer, _, _) => sequenceSliceOf(sequenceId, pointer)
//        }
...
//                            (preprocessed.baseProgresses.toList map { c => ProgressTask(tip.asInstanceOf[SequenceKernel], c.shiftGen(gen)) }) ++
