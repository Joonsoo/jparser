...
object DirectionalTopDownParsing {
                    ...
                    val predictingStacks = context.stacks.zipWithIndex flatMap { x =>
                        ...
                        stack.predictionStack.head match {
                            case PredictionStackNonterminal(nonterminal @ Nonterminal(name)) =>
                                val analyses = context.grammar.rules(name).zipWithIndex map { rhsIdx =>
                                    val (rhs, idx) = rhsIdx
    ...
    class InvalidActionException(msg: String) extends Exception(msg)

    private def initialContextOf(grammar: ContextFreeGrammar, input: List[Inputs.Input]): PredictionStackContext = {
    ...
    // Breadth-first
    def breadthFirst(grammar: ContextFreeGrammar, input: List[Inputs.Input]): (PredictionStackContext, DirectionalParser) = {
        (initialContextOf(grammar, input), new DirectionalTopDownParser {
            override def next(context: PredictionStackContext, nextAction: NextAction): PredictionStackContext = {
                if (this.nextAction(context) != nextAction) {
                    throw new InvalidActionException("nextAction modified")
                }
                super.next(context, nextAction)
            }
        })
    }

    // Depth-first
    def depthFirst(grammar: ContextFreeGrammar, input: List[Inputs.Input]): (PredictionStackContext, DirectionalParser) = {
        (initialContextOf(grammar, input), ???)
    }
