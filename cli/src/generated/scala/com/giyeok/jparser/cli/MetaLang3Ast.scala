...
import com.giyeok.jparser.ParseResultTree.{BindNode, Node, SequenceNode, TerminalNode}
...
object MetaLang3Ast {
  ...
  def matchAtom(node: Node): Atom = {
    ...
    val v79 = v53.id match {
      ...
      case 384 =>
        ...
        assert(v59.id == 385)
        matchFuncCallOrConstructExpr(v60)
      case 369 =>
  ...
  def matchAtomSymbol(node: Node): AtomSymbol = {
    ...
    val v103 = v80.id match {
      ...
      case 289 =>
        ...
        assert(v95.id == 63)
        matchNonterminal(v96)
      case 290 =>
        val v97 = v81.asInstanceOf[SequenceNode].children(2)
        val BindNode(v98, v99) = v97
        assert(v98.id == 291)
        matchInPlaceChoices(v99)
      case 302 =>
  ...
  val milestoneParserData = MilestoneParserProtobufConverter.convertProtoToMilestoneParserData(
    MilestoneParserDataProto.MilestoneParserData.parseFrom(getClass.getResourceAsStream("/cdglang3.pb")))
  ...
  def parseAst(text: String): Either[Grammar, ParsingErrors.ParsingError] =
    parse(text) match {
      case Left(forest) =>
        println(s"Trees: ${forest.trees.size}")
