...
object MlProtoAst {
  ...
  def main(args: Array[String]): Unit = {
    val source = "module Conv2d<>(inChannels: Int, outChannels: Int) {}"
    val x = parseAst(source).left.get
    println(source)
    x.modules.head.params.foreach { param =>
      val hl = param.name
      println((" " * hl.parseNode.start) + ("-" * (hl.parseNode.end - hl.parseNode.start)))
    }
    println(x)
  }
}
