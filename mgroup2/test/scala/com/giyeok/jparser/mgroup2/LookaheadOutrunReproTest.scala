...
// bug-B (negative lookahead discharge) 재현 핀. 실행: bibix4 runMgroup2LookaheadReproTest
//
...
class LookaheadOutrunReproTest extends AnyFlatSpec {
  ...
  private lazy val grammar: NGrammar = MetaLanguage3.analyzeGrammar(grammarText, "Testing").ngrammar
  private lazy val parserData = new MilestoneGroupParserGen(grammar).parserData()
  ...
  private def mgroup2Accepts(inputs: List[Inputs.Input]): Boolean = {
  ...
  "fn(); with guarded body outrunning the lookahead" should "be REJECTed in agreement with naive2 (bug B pin)" in {
    ...
    assert(!mgroup2Accepts(inputs),
      "bug B 재현: mgroup2가 fn();를 오수락했다 — cab1092e revert 이후 재설계 랜딩까지는 RED가 의도된 상태다")
  }
