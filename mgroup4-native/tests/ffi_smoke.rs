//! Smoke test for the C ABI in `src/ffi.rs`. Loads `libmgroup4_native.dylib`
...
fn dylib_path() -> PathBuf {
    ...
    let release = crate_dir.join("target/release/libmgroup4_native.dylib");
    let debug = crate_dir.join("target/debug/libmgroup4_native.dylib");
    if release.exists() {
        release
...
fn fixture_data_pb() -> PathBuf {
    ...
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../mgroup3-native/tests/fixtures/parser/single_char/data.pb")
}
...
fn ffi_smoke_roundtrip() {
    ...
    let version: Symbol<NativeVersion> =
        unsafe { lib.get(b"mgroup4_native_version").expect("symbol native_version") };
    let new_from_file: Symbol<ParserNewFromFile> = unsafe {
        lib.get(b"mgroup4_parser_new_from_file").expect("symbol new_from_file")
    };
    let parse: Symbol<ParserParse> =
        unsafe { lib.get(b"mgroup4_parser_parse").expect("symbol parse") };
    ...
    let parser_free: Symbol<ParserFree> =
        unsafe { lib.get(b"mgroup4_parser_free").expect("symbol parser_free") };
