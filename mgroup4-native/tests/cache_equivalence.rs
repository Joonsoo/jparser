...
use mgroup4_native::parser::{Mgroup4Parser, ParsingError};
use mgroup4_native::parser_cache;
...
fn committed_fixtures_root() -> PathBuf {
    // fixture 는 mgroup3-native 의 것을 읽기 전용 참조 (복사 금지 — 사용자 제약).
    // 캐시 쓰기는 tempdir 로만 하므로 fixture 는 오염되지 않는다.
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../mgroup3-native/tests/fixtures/parser")
}
...
fn serialize_result(parser: &Mgroup4Parser, input: &str) -> String {
...
fn proto_and_cache_paths_agree() {
    ...
    for case_dir in &case_dirs {
        ...
        let cold_parser = Mgroup4Parser::from_plain(plain_cold);

        let plain_warm =
            parser_cache::load_plain_from_file(&tmp_pb).expect("warm cache load");
        let warm_parser = Mgroup4Parser::from_plain(plain_warm);
...
fn corrupt_cache_falls_back() {
    ...
    let parser = Mgroup4Parser::from_plain(plain);
    let proto_parser =
        Mgroup4Parser::new(Mgroup3ParserData::decode(bytes.as_slice()).unwrap());
    ...
    let _ = Mgroup4Parser::from_plain(plain2);
}
...
fn degenerate_cache_files_fall_back() {
    ...
    let bytes = fs::read(case_dir.join("data.pb")).expect("read data.pb");
    let proto_start = Mgroup4Parser::new(Mgroup3ParserData::decode(bytes.as_slice()).unwrap())
        .start_symbol_id();
