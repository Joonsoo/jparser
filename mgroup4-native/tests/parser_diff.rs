...
//!
//! ★ mgroup4-native fixture 규약: 이 crate 은 fixture 를 **복사하지 않고**
//! `../mgroup3-native/tests/fixtures/` 의 것을 **읽기 전용으로 참조**한다
//! (사용자 제약: mgroup3-native 무변경 + fixture read-only). golden 은
//! mgroup4(n=1) ≡ mgroup3 (출력 불변)이므로 재생성 없이 그대로 byte-compare 한다.
//! 재생성이 필요하면 그건 버그다.
//!
...
//!
//! Each fixture under `<mgroup3-native>/tests/fixtures/parser/<case>/` has:
...
//!
//! ★ n 파라미터: env `MG4_INTERIOR_N` (Kotlin 과 동일 이름). 미설정 시 n=1
//! (≡ mgroup3). n∈{2,4,6} 재실행도 **같은 golden 과 byte-identical** 이어야 한다
//! (출력 불변이 group 기계의 정체성 — B1b 게이트 (i)).
...
/// mgroup3-native 의 fixture 루트 (읽기 전용 참조 — 복사 금지, 상대경로).
/// mgroup4-native 와 mgroup3-native 는 리포 루트의 형제 디렉토리다.
fn mgroup3_native_fixtures() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../mgroup3-native/tests/fixtures")
}

fn committed_fixtures_root() -> PathBuf {
    mgroup3_native_fixtures().join("parser")
}

fn generated_fixtures_root() -> PathBuf {
    mgroup3_native_fixtures().join("parser_generated")
}

/// n = env `MG4_INTERIOR_N` (미설정 시 1 ≡ mgroup3).
fn interior_n() -> i32 {
    std::env::var("MG4_INTERIOR_N")
        .ok()
...
fn run_case(case_dir: &std::path::Path) -> Vec<String> {
    ...
    // n = env MG4_INTERIOR_N (미설정 시 1 ≡ mgroup3). 모든 n 에서 같은 golden 과
    // byte-identical 이어야 한다 (출력 불변 = group 기계의 정체성 — B1b 게이트 (i)).
    let parser = Mgroup4Parser::new_with_n(data, interior_n());
