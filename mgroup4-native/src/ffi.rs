...
//!
//! Memory ownership:
//! - `mgroup4_parser_new*` returns a `*mut Mgroup4Parser` allocated with
//!   `Box::into_raw`. Free with `mgroup4_parser_free`.
//! - `mgroup4_parser_parse` writes a `(ptr, len)` pair pointing at a heap
//!   buffer the callee allocated via `Box::into_raw(boxed_slice)`. The
//!   caller must release it with `mgroup3_free_buffer`.
//!
//! Threading:
//! - `Mgroup4Parser` is not thread-safe (a `RefCell` cache lives inside).
//!   The caller must not invoke `mgroup4_parser_parse` concurrently on the
...
use crate::parser::{encode_parse_result, Mgroup4Parser};
...
#[unsafe(no_mangle)]
pub extern "C" fn mgroup4_parser_new(
    data_bytes: *const u8,
    data_len: usize,
    err: *mut i32,
) -> *mut Mgroup4Parser {
...
#[unsafe(no_mangle)]
pub extern "C" fn mgroup4_parser_new_from_file(
    path: *const c_char,
    err: *mut i32,
) -> *mut Mgroup4Parser {
    let result = catch_unwind(AssertUnwindSafe(|| {
...
/// Like `mgroup4_parser_new_from_file`, but uses a sibling rkyv cache
...
#[unsafe(no_mangle)]
pub extern "C" fn mgroup4_parser_new_from_file_cached(
    path: *const c_char,
    err: *mut i32,
) -> *mut Mgroup4Parser {
    let result = catch_unwind(AssertUnwindSafe(|| {
        ...
        Ok(Mgroup4Parser::from_plain(plain))
    }));
    match result {
        Ok(Ok(parser)) => {
            write_err(err, MGROUP3_OK);
...
/// null arg).
#[unsafe(no_mangle)]
pub extern "C" fn mgroup4_parser_parse(
    parser: *mut Mgroup4Parser,
...
/// Free a buffer previously returned via `mgroup4_parser_parse`.
#[unsafe(no_mangle)]
...
/// Drop a parser handle previously returned by one of the `mgroup4_parser_new*`
/// functions.
#[unsafe(no_mangle)]
pub extern "C" fn mgroup4_parser_free(parser: *mut Mgroup4Parser) {
    if parser.is_null() {
        return;
    }
    let _ = catch_unwind(AssertUnwindSafe(|| unsafe {
...
/// build.
#[unsafe(no_mangle)]
pub extern "C" fn mgroup4_native_version() -> *const c_char {
