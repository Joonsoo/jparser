...
import com.giyeok.jparser.nparser.{NGrammar, ParseTreeConstructor}
...
class BasicParseTest(val testsSuite: Traversable[GrammarTestCases]) extends FlatSpec {
    ...
    testsSuite foreach { test =>
        ...
        test.samples.validInputs foreach { input => testCorrect(test, Inputs.fromString(input)) }
        test.samples.invalidInputs foreach { input => testIncorrect(test, Inputs.fromString(input)) }
        test.samples.ambiguousInputs foreach { input => testAmbiguous(test, Inputs.fromString(input)) }
    }
}
