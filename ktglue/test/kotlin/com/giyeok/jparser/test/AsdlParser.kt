...
import com.giyeok.jparser.ktlib.Milestone2ParserLoader.loadParserFromResource
import com.giyeok.jparser.ktlib.toKtList

object AsdlParser {
  val parser = loadParserFromResource("/asdl-parserdata.pb")

  fun parse(source: String): Asdl.ModuleDef {
    ...
    val kernels = parser.kernelsHistory(parseResult).toKtList()
      .map { KernelSet(it) }
    val astifier = Asdl(inputs.toKtList(), kernels)
    return astifier.matchStart()
  }

  @JvmStatic
  fun main(args: Array<String>) {
    val pythonAsdl = parse("""
      module Python
      {
